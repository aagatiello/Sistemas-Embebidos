...
// Lista de comandos
const char cmd1[] = {"5001AA"}; // set MIC2_ledgreen 1
const char cmd2[] = {"5000AA"}; // set MIC2_ledgreen 0
const char cmd3[] = {"5101AA"}; // set MIC2_ledgred 1
const char cmd4[] = {"5100AA"}; // set MIC2_ledgred 0
const char cmd5[] = {"5201AA"}; // print MIC2_data
const char cmd6[] = {"5200AA"}; // stop MIC2_data
...
int main(void)
{
    ...
    while (1) {
        ...
        // Rutina de actualización de datos mediante UART_TX ISR
        if ((Allowprint)&&(BufferLoadDone)&&(U1STAbits.TRMT)) 
        {
            if (U1_PrintRate_ISR++ >= 4) // Ritmo de transmision de datos de la Uart1 Tx: 4 veces delay_ms(200)
            {
                memset(txbuffer_ISR, '\0', sizeof (txbuffer_ISR)); 
                sprintf(txbuffer_ISR, "IMPRIMIENDO DATOS: %d\r", contador++);
                ...
                if (contador == 101) contador = 0;
            }
        }

        if (!(Allowprint)&&(BufferLoadDone)&&(U1STAbits.TRMT) && flag)
        {
            memset(txbuffer_ISR, '\0', sizeof (txbuffer_ISR));
            sprintf(txbuffer_ISR, "IMPRESION DE DATOS DETENIDA\r");
